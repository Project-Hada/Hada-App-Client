 ...
 * users can only affect their mastery level in a session 24 hours a part from the last session to stop them from spamming.
 * 0.) using a set[] of cards, if less than 24 hours from last session, record the start time and enable grading
 ...
 * 6.) when restarting the session, do not grade mastery if the start time of the session is less than 24 hours from the last
 *
 ...
 * 
 * Playlist Parameters:
 * l: last started session, rewritten every new started session >= 24hours from last 
 * 
 * Card Parameters:
